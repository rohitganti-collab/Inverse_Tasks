...
**The shortcut solver must fail.** It exists to prove your trap traps, and the
validator treats a passing shortcut as a failed task. It must fail *onto the named
near-miss* — that's what makes the concentration check mean anything.
...
**Never name the trap or the method** — not in `problem.md`, not in an action
description, not in `help()` output, not in anything the oracle returns.
...
**Don't document the tool call syntax in your prompt.** The container appends a
calling guide generated from your `ACTIONS`. Name the modes; leave the mechanics
to the guide.

### The oracle contract

Both directions use the same one — the engine doesn't special-case direction.
